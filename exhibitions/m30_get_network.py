...
with open(sys.argv[1], 'r') as f:
	...
	for record in sc:
		if record[2] not in exhib_dict:
			exhib_dict[record[2]] = []
		exhib_dict[record[2]].append([record[0], record[1], record[6], record[10]])
print(exhib_dict)
c = pymongo.MongoClient()
cursor = c.myproject.Artwork.find({'temporary_exhibitions_m30':{'$exists':True}},{'temporary_exhibitions_m30':1, 'ensemble_id':1, 'title_list':1, 'authors_list':1})
g = open(sys.argv[2], 'w')
dc = csv.writer(g)
dc.writerow(['Artwork id', 'Ensemble id', 'Author', 'Title', 'Exhibition (champs brut)', 'raw_md5', 'uniq_md5', 'start date', 'end date'])
for doc in cursor:
	for art_exhibition in get_list_from_html(doc['temporary_exhibitions_m30']):
